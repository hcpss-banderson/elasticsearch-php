...
class BuildAction
{
    ...
    private function do(array $actions): string
    {
        ...
        foreach ($actions as $key => $value) {
            ...
            if (method_exists($this, (string) $key)) {
                $this->$key($value, $vars);
            } else {
                // headers
        ...
        if (isset($this->clientParams['ignore'])) {
            ...
            if (is_array($vars[':code'])) {
                return PhpUnitTests::render(self::TEMPLATE_ENDPOINT_TRY_CATCH_ARRAY, $vars);
            }
            return PhpUnitTests::render(self::TEMPLATE_ENDPOINT_TRY_CATCH, $vars);
        }
        return PhpUnitTests::render(self::TEMPLATE_ENDPOINT, $vars);
    }
    ...
    private function transform_and_set(array $action): string
    {
        ...
        if (false !== strpos($transform, '#base64EncodeCredentials')) {
            ...
            $param2 = $matches[2][0];
            $this->variables[] = $key;
            return PhpUnitTests::render(self::TEMPLATE_TRANSFORM_AND_SET, [
                ':var'   => $key,
    ...
    private function set(array $action): string
    {
        ...
        foreach ($action as $key => $var) {
            $this->variables[] = $var;
            $output .= PhpUnitTests::render(self::TEMPLATE_SET_VARIABLE, [
                ':var'   => $var,
    ...
    {
        $vars[':response-check'] .= PhpUnitTests::render(self::TEMPLATE_WARNINGS, [
            ':expected' => $action
        ]);
    }
    ...
    {
        $vars[':response-check'] .= PhpUnitTests::render(self::TEMPLATE_ALLOWED_WARNINGS, [
            ':expected' => $action
        ]);
    }
    ...
    private function catch(string $action, array &$vars)
    {
        switch ($action) {
            ...
            case 'unavailable':
                $expectedException = ElasticsearchException::class;
                $scriptException = PhpUnitTests::render(self::TEMPLATE_CATCH_UNAVAILABLE);
                break;
            case 'param':
                $expectedException = ElasticsearchException::class;
    ...
    private function match(array $actions)
    {
        $key = key($actions);
        if (null === $actions[$key]) {
            return PhpUnitTests::render(self::TEMPLATE_IS_NULL, [
                ':value' => $this->convertResponseField($key)
            ]);
        }
        ...
        return PhpUnitTests::render(self::TEMPLATE_MATCH_EQUAL, $vars);
    }
    ...
    private function is_true(string $value) 
    {
        ...
        if ($vars[':value'] === '$response') {
            return PhpUnitTests::render(self::TEMPLATE_IS_TRUE_RESPONSE, $vars);
        }
        return PhpUnitTests::render(self::TEMPLATE_IS_TRUE, $vars);
    }
    ...
    private function is_false(string $value) 
    {
        ...
        if ($vars[':value'] === '$response') {
            return PhpUnitTests::render(self::TEMPLATE_IS_FALSE_RESPONSE, $vars);
        }
        return PhpUnitTests::render(self::TEMPLATE_IS_FALSE, $vars);
    }
    ...
    private function length(array $actions)
    {
        ...
        return PhpUnitTests::render(self::TEMPLATE_LENGTH, [
    ...
    private function skip(array $actions)
    {
        if (isset($actions['version']) && isset($actions['reason'])) {
            ...
            if (strtolower($version[0]) === 'all' || 
               (version_compare(PhpUnitTests::$esVersion, $version[0], '>=') && version_compare(PhpUnitTests::$esVersion, $version[1], '<='))
            ) {
                $this->skippedTest = true;
                return PhpUnitTests::render(self::TEMPLATE_SKIP_VERSION, [
                    ':testname'  => "__CLASS__ . '::' . __FUNCTION__",
                    ':esversion' => sprintf("'%s'", PhpUnitTests::$esVersion),
        ...
        if (isset($actions['features'])) {
            ...
            foreach ($features as $feature) {
                if (!in_array($feature, self::SUPPORTED_FEATURES)) {
                    $this->skippedTest = true;
                    return PhpUnitTests::render(self::TEMPLATE_SKIP_FEATURE, [
                        ':testname' => "__CLASS__ . '::' . __FUNCTION__",
                ...
                switch ($feature) {
                    case 'xpack': 
                        if (PhpUnitTests::$testSuite !== 'platinum') {
                            $this->skippedTest = true;
                            return PhpUnitTests::render(self::TEMPLATE_SKIP_XPACK, [
                                ':testname' => "__CLASS__ . '::' . __FUNCTION__"
                            ]);
                        } 
                        break;
                    case 'no_xpack': 
                        if (PhpUnitTests::$testSuite !== 'free') {
                            $this->skippedTest = true;
                            return PhpUnitTests::render(self::TEMPLATE_SKIP_OSS, [
                                ':testname' => "__CLASS__ . '::' . __FUNCTION__"
                            ]);
                        } 
                        break;
    ...
    {
        $key = key($actions);
        return PhpUnitTests::render(self::TEMPLATE_GT, [
            ':expected' => $actions[$key],
            ':value' => $this->convertResponseField($key)
        ]);
    }
    ...
    {
        $key = key($actions);
        return PhpUnitTests::render(self::TEMPLATE_GTE, [
            ':expected' => $actions[$key],
            ':value' => $this->convertResponseField($key)
        ]);
    }
    ...
    {
        $key = key($actions);
        return PhpUnitTests::render(self::TEMPLATE_LT, [
            ':expected' => $actions[$key],
            ':value' => $this->convertResponseField($key)
        ]);
    }
    ...
    {
        $key = key($actions);
        return PhpUnitTests::render(self::TEMPLATE_LTE, [
            ':expected' => $actions[$key],
            ':value' => $this->convertResponseField($key)
        ]);
    }
